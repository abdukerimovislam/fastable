...
class ProfileScreen extends StatelessWidget {
  ...
  void _handleDeleteAccount(BuildContext context, AppLocalizations l10n) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(l10n.confirmDeleteMsg, style: const TextStyle(color: Colors.white70)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text(l10n.cancel, style: const TextStyle(color: Colors.blueAccent))),
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
  ...
  void _showConflictDialog(BuildContext context, AppLocalizations l10n) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(l10n.dialogSyncConflictContent, style: const TextStyle(color: Colors.white70)),
        actions: [
          TextButton(onPressed: () async { Navigator.pop(ctx); await _resolve(context, merge: false, l10n: l10n); }, child: Text(l10n.btnOverwriteLocal, style: const TextStyle(color: Colors.grey))),
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.blueAccent), onPressed: () async { Navigator.pop(ctx); await _resolve(context, merge: true, l10n: l10n); }, child: Text(l10n.btnMergeData, style: const TextStyle(color: Colors.white))),
        ],
      ),
    );
  }
  ...
  void _showHeightPicker(BuildContext context, double currentHeight, AppLocalizations l10n) {
    ...
    showRouletteSheet<int>(context: context, title: l10n.selectHeight, items: List.generate(151, (index) => 100 + index), initialItem: currentHeight.toInt().clamp(100, 250), textMapper: (val) => "$val cm", onSave: (val) => context.read<WeightBloc>().add(UpdateHeight(val.toDouble())));
  }
  ...
  void _showWeightPicker(BuildContext context, double currentWeight, AppLocalizations l10n) {
    ...
    showRouletteSheet<double>(context: context, title: l10n.selectWeight, items: List.generate(2700, (index) => 30.0 + (index * 0.1)), initialItem: current, textMapper: (val) => "${val.toStringAsFixed(1)} ${l10n.unitKg}", onSave: (val) => context.read<WeightBloc>().add(AddWeightEntry(val)));
  }

  void _showAgePicker(BuildContext context, int currentAge, AppLocalizations l10n) {
    getIt<HapticService>().mediumImpact();
    showRouletteSheet<int>(context: context, title: l10n.selectAge, items: List.generate(91, (index) => 10 + index), initialItem: currentAge.clamp(10, 100), textMapper: (val) => "$val", onSave: (val) => context.read<WeightBloc>().add(UpdateAge(val)));
  }

  void _showGenderPicker(BuildContext context, AppLocalizations l10n) {
    showModalBottomSheet(context: context, backgroundColor: const Color(0xFF1E1E1E).withOpacity(0.95), shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (ctx) => Padding(padding: const EdgeInsets.all(24.0), child: Column(mainAxisSize: MainAxisSize.min, children: [Text(l10n.selectGender, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 20), _buildOptionItem(ctx, l10n.genderMale, () => context.read<WeightBloc>().add(const UpdateGender(Gender.male))), _buildOptionItem(ctx, l10n.genderFemale, () => context.read<WeightBloc>().add(const UpdateGender(Gender.female))), const SizedBox(height: 20)])));
  }

  void _showActivityPicker(BuildContext context, AppLocalizations l10n) {
    showModalBottomSheet(context: context, backgroundColor: const Color(0xFF1E1E1E).withOpacity(0.95), shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (ctx) => Padding(padding: const EdgeInsets.all(24.0), child: Column(mainAxisSize: MainAxisSize.min, children: [Text(l10n.selectActivity, style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 20), _buildOptionItem(ctx, l10n.activitySedentary, () => context.read<WeightBloc>().add(const UpdateActivityLevel(ActivityLevel.sedentary))), _buildOptionItem(ctx, l10n.activityModerate, () => context.read<WeightBloc>().add(const UpdateActivityLevel(ActivityLevel.moderate))), _buildOptionItem(ctx, l10n.activityActive, () => context.read<WeightBloc>().add(const UpdateActivityLevel(ActivityLevel.active))), const SizedBox(height: 20)])));
  }

  Widget _buildOptionItem(BuildContext context, String title, VoidCallback onTap) {
    return ListTile(title: Text(title, style: const TextStyle(color: Colors.white, fontSize: 18)), onTap: () { getIt<HapticService>().selectionClick(); onTap(); Navigator.pop(context); }, trailing: const Icon(Icons.check_circle_outline, color: Colors.white24));
  }
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(l10n.navProfile, style: const TextStyle(color: Colors.white, fontSize: 34, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
